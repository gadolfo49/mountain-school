...
check_200 "$BASE/?smoke=r5-6-mounty-text-assist"
grep -q 'MF-R5.6-MOUNTY-TEXT-ASSIST-20260912' /tmp/mf_body
...
grep -q 'mounty-guide-v1.js?v=20260912-r5-6' /tmp/mf_body
grep -q 'mounty-assist-v1.js?v=20260912-r5-6' /tmp/mf_body
grep -q 'final-flow-v1.js?v=20260912-r5-6' /tmp/mf_body
...
check_200 "$BASE/mounty-guide-v1.js?v=20260912-r5-6"
...
check_200 "$BASE/mounty-assist-v1.js?v=20260912-r5-6"
grep -q 'Ayudar con Mounty' /tmp/mf_body
grep -q 'Preparar texto con Mounty' /tmp/mf_body
grep -q 'Compartir ahora' /tmp/mf_body
grep -q 'Puedes seguir preparando el texto' /tmp/mf_body
grep -q 'mf_broadcast_preview_count' /tmp/mf_body
grep -q "purpose:'observation'" /tmp/mf_body
grep -q "purpose:'planning'" /tmp/mf_body
grep -q "purpose:'incident'" /tmp/mf_body
check_200 "$BASE/privacy.html?smoke=r5-6"
grep -q 'Política de privacidad' /tmp/mf_body
check_200 "$BASE/account-deletion.html?smoke=r5-6"
grep -q 'Solicitud de eliminación de cuenta' /tmp/mf_body
check_200 "$BASE/styles-a11y-v1.css?smoke=r5-6"
grep -q ':focus-visible' /tmp/mf_body
check_200 "$BASE/sw.js?smoke=r5-6"
grep -q 'mountain-family-shell-r5-6-mounty-text-assist-20260912' /tmp/mf_body
grep -q "'./mounty-assist-v1.js'" /tmp/mf_body
...
echo 'LIVE_SMOKE_OK_R5_6_MOUNTY_TEXT_ASSIST'